...
class RelationalGCNClassifier(nn.Module):
    ...
    def __init__(self, num_relations: int, num_classes: int, feature_specs: Mapping[str, FeatureSpec],
                 ...
                 dropout: float = 0.2, num_bases: int = 30, rgcn_backend: str = "fast", **_) -> None:
        super().__init__()
        if num_layers != 2:
            raise ValueError("Current sampled training uses exactly two message-passing layers")
        if rgcn_backend not in {"fast", "standard"}:
            raise ValueError("rgcn_backend must be 'fast' or 'standard'")
        conv_class = FastRGCNConv if rgcn_backend == "fast" else RGCNConv
        self.rgcn_backend = rgcn_backend
        self.feature_encoder = NodeFeatureEncoder(feature_specs, branch_dim, hidden_dim, dropout)
        self.convs = nn.ModuleList(
            conv_class(hidden_dim, hidden_dim, num_relations=num_relations, num_bases=num_bases)
